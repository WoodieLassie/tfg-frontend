<template>
    <section v-if="episode">
        <section class="episode__information">
            <article class="episode__header">
                <h1>
                {{ episode.episodeNum }}
                </h1>
                <!-- <img v-if="show.images" :src="show.images[0].url" :alt="show.name"> -->
            </article>
            <article class="episode__body">
                <p>{{ episode.description }}</p>
            </article>
        </section>
        <section class="episode__characters">
            <h2>Personajes que aparecen</h2>
            <article v-for = "character in episode.characters" class="episode__character">
                <div class="episode__character__info">
...
<script>
    ...
    export default {
        ...
        async created() {
            const episodeData = await fetch(`http://localhost:8080/api/episodes/${this.$props.id}`)
